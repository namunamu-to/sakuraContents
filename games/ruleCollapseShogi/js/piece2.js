...
class Piece {
    ...
    checkCanMove(toX, toY) {
        if (toX < 0 || toX >= 9 || toY < 0 || toY >= 9) return false; //インデックスが盤面の外側か
        else if (nowBoard[toY][toX] == "") return true;
        else if (nowBoard[toY][toX].isRedTeam == this.isRedTeam) return false; //移動先に味方がいるか
    ...
    movePreview() {
        //桂馬の場合
        if (this.role == "桂馬") {
            const toX1 = this.nowXTile - 1;
            const toX2 = this.nowXTile + 1;
            const toY = this.nowYTile - 2;
            if (this.checkCanMove(toX1, toY)) this.makeCanMoveTile(toX1, toY);
            if (this.checkCanMove(toX2, toY)) this.makeCanMoveTile(toX2, toY);
        ...
        for (let moveKey of moveKeys) { //8方向の移動範囲を処理
            console.log(`------------------${this.role}${moveKey}--------------------`);
            for (let i = 0; i < mobilitys[this.role][moveKey]; i++) { //方向に対する移動量、分繰り返す
                // console.log(this.nowXTile + moveDir[moveKey][0] * i);
                
                //チェックする座標
                const toX = this.nowXTile + (moveDir[moveKey][0] * (i + 1));
                const toY = this.nowYTile + (moveDir[moveKey][1] * (i + 1));
                console.log(nowBoard[toX][toY], typeof(nowBoard[toX][toY]), toX, toY);

                //移動先に応じてタイルを置くか決める
                if (!this.checkCanMove(toX, toY)) break; //移動できないならbreak
                
                this.makeCanMoveTile(toX, toY);
                // console.log(this.checkCanMove(toX, toY), toX, toY);
                // if (nowBoard[toX][toY] === "") break; //敵チームならbreak
                // } else break;
